...
const StudentFees = () => {
	...
	const [feeData, setFeeData] = useState({ accounts: [], payments: [] });
	const [error, setError] = useState("");

	useEffect(() => {
		if (!token) return;
		api.get("/api/student/profile", { headers: { Authorization: `Bearer ${token}` } }).then((res) => setStudent(res.data)).catch(() => setStudent(null));
		api.get("/api/fees/student", { headers: { Authorization: `Bearer ${token}` } }).then((res) => setFeeData(res.data || { accounts: [], payments: [] })).catch(() => setError("Failed to load fee records."));
	}, [token]);

	const totals = feeData.accounts.reduce((summary, account) => ({
	...
	return (
		<div className="min-h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans">
			...
			<main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
				...
				<section className="bg-gradient-to-br from-emerald-600 to-teal-700 text-white rounded-2xl p-6 sm:p-8 shadow-lg"><p className="text-xs font-bold uppercase tracking-widest text-emerald-100">{student ? `${student.firstname || ""} ${student.lastname || ""}`.trim() : "Student account"}</p><h2 className="text-2xl font-black mt-2">Your fee summary</h2><div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6"><Summary label="Total fees" value={`₹${totals.total.toFixed(2)}`} icon={Wallet} /><Summary label="Paid fees" value={`₹${totals.paid.toFixed(2)}`} icon={CircleDollarSign} /><Summary label="Remaining fees" value={`₹${totals.remaining.toFixed(2)}`} icon={Wallet} /></div></section>
				<section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2"><CalendarDays className="w-4 h-4 text-emerald-500" /><h2 className="text-xs font-bold uppercase tracking-wider text-slate-500">Payment history</h2></div>{feeData.payments.length === 0 ? <div className="p-10 text-center"><CreditCard className="w-9 h-9 mx-auto text-slate-300 dark:text-slate-700" /><p className="text-sm font-semibold text-slate-500 mt-3">No payment records available yet.</p></div> : <div className="divide-y divide-slate-100 dark:divide-slate-800">{feeData.payments.map((payment) => <div key={payment.payment_id} className="flex items-center justify-between gap-4 px-5 py-4"><div><p className="text-sm font-bold">{payment.course_code} · {payment.semoryear}</p><p className="text-xs text-slate-500 mt-1">{new Date(payment.paid_at).toLocaleDateString()}</p></div><span className="text-sm font-black text-emerald-600">₹{Number(payment.amount).toFixed(2)}</span></div>)}</div>}</section>
